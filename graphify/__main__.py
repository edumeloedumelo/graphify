...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("    --graph <path>          path to graph.json (default graphify-out/graph.json)")
        print("  jarvis [\"question\"]     conversational assistant over the graph (REPL, or one-shot with a question)")
        print("    --graph <path>          path to graph.json (default graphify-out/graph.json)")
        print("    --no-llm                deterministic graph answers only (skip LLM synthesis)")
        print("    --backend=<name>        LLM backend for grounded answers (default: auto-detect)")
        print("    --model=<name>          model to use for grounded answers")
